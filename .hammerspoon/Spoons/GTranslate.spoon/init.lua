...
function mod:gtranslate()
  ...
  function setLang(so, ta)
    ...
    alerts["langPrimary"] = hs.alert.show(
      string.format("%s ⇢ %s", string.upper(source), string.upper(target)),
      { ["textSize"] = 50 },
      2
    )
...
function mod:init()
  mod.APIKEY = APIKEY
  mod.source = source or "en"
  mod.target = target or "es"

  mods = mods or { "cmd", "alt", "shift" }
  key = key or "T"
