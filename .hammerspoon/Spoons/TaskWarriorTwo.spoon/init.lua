      ...
      if file:sub(-12) == "pending.data" then
        ...
        for line in io.lines(file) do
          if (string.match(line, "start")) then
            ...
            taskname = string.match(line, "description:" .. '"' .. '([a-zA-Z%s]*)' .. '"')
            foundtask = true
          end
        end
      end
...
function obj:tick_timer_fn()
  return hs.timer.doEvery(60, function() 
    ...
    if self.elapsed_minutes % 20 == 0 then
      sounds = hs.sound.systemSounds()
      glass = hs.sound.getByName(sounds[9])
      glass:play()
    end
...
function obj:checkIfTaskStarted()
  ...
  for line in lines do
    ...
    if (string.match(line, "start")) then
      ...
      taskname = string.match(line, '"' .. '([a-zA-Z%s%d]*)' .. '"')
